...
export function MailIndex() {
  ...
  function loadUnreadMails() {
    ...
    mailService.query(unreadFilter).then((allInboxMails) => {
      const unread = allInboxMails.filter(
        (mail) => mail.to === 'user@appsus.com' && mail.isRead === false && mail.removedAt === null
      )
  ...
  return (
    <section className="main-container">
      ...
      <MailFolderList
        ...
        isMini={isMini}
        onSetIsMini={onSetIsMini}
      />
      <section className="logo-menu ">
        <span className="material-symbols-outlined menu-btn btn-toggle-menu" onClick={() => onSetIsMini()}>
          menu
        </span>
      </section>
